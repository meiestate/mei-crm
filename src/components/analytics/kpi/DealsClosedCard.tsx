...
export type DealsClosedBreakdownItem = {
  label: string;
  value: number;
  displayValue?: string;
  percent: number;
...
export type DealsClosedCardProps = {
  title?: string;
  subtitle?: string;
  totalClosed?: number;
  wonDeals?: number;
  ...
  lastUpdated?: string;
  loading?: boolean;
  breakdown?: DealsClosedBreakdownItem[];
  onClick?: () => void;
};
...
function getTargetProgress(totalClosed: number, targetDeals: number): number {
  if (!Number.isFinite(targetDeals) || targetDeals <= 0) return 0;
  return clampPercent((totalClosed / targetDeals) * 100);
}

const DEFAULT_BREAKDOWN: DealsClosedBreakdownItem[] = [
  {
    label: "Direct Sales",
    value: 18,
    displayValue: "18",
...
export default function DealsClosedCard({
  title = "Deals Closed",
  subtitle = "Total closures completed across the active sales period",
  totalClosed = 42,
  wonDeals = 34,
  lostDeals = 6,
  pendingClosures = 2,
  ...
  onClick,
}: DealsClosedCardProps) {
  const toneStyles = useMemo(() => getToneStyles(tone), [tone]);
  const trendStyles = useMemo(() => getTrendStyles(trend), [trend]);

  const progress = useMemo(
    () => getTargetProgress(totalClosed, targetDeals),
    [totalClosed, targetDeals]
  );

  const gapToTarget = useMemo(
    () => Math.max(targetDeals - totalClosed, 0),
    [targetDeals, totalClosed]
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Monthly target: {formatInteger(targetDeals)} deals
        </span>
      ...
      >
        ...
        <MetricTile
          label="Lost"
          value={formatInteger(lostDeals)}
          tone={toneStyles}
        />
        <MetricTile
          label="Gap to Target"
          value={formatInteger(gapToTarget)}
          tone={toneStyles}
        />
      </div>
      ...
      >
        <MiniStatPill
          ...
          value={formatInteger(pendingClosures)}
          tone={toneStyles}
        />
        <MiniStatPill
          label="Win Ratio"
          value={
            totalClosed > 0
              ? formatPercent((wonDeals / totalClosed) * 100)
              : "0%"
          }
          tone={toneStyles}
        />
            ...
            >
              <div style={{ minWidth: 0 }}>
                ...
                >
                  ...
                  >
                    {item.displayValue ?? formatInteger(item.value)}
                  </span>
                </div>
              ...
              >
                {formatPercent(clampPercent(item.percent))}
              </div>
            </div>
          ))}
        </div>
      ...
      >
        ...
        >
          Closure health: {totalClosed >= targetDeals ? "Ahead of plan" : "Still building"}
        </span>
      </div>
    </section>
  );

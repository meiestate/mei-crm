...
export type LeadsBreakdownItem = {
  label: string;
  count: number;
  percent: number;
  color?: string;
...
export type TotalLeadsCardProps = {
  title?: string;
  subtitle?: string;
  totalLeads?: number;
  targetLeads?: number;
...
function getTargetProgress(totalLeads: number, targetLeads: number): number {
  if (!Number.isFinite(targetLeads) || targetLeads <= 0) return 0;
  return clampPercent((totalLeads / targetLeads) * 100);
}

const DEFAULT_BREAKDOWN: LeadsBreakdownItem[] = [
  { label: "Website", count: 148, percent: 32, color: "#2563eb" },
  { label: "Meta Ads", count: 116, percent: 25, color: "#7c3aed" },
  { label: "WhatsApp Campaigns", count: 84, percent: 18, color: "#10b981" },
  { label: "Referrals", count: 69, percent: 15, color: "#f59e0b" },
  { label: "Walk-ins", count: 46, percent: 10, color: "#ef4444" },
];

export default function TotalLeadsCard({
  title = "Total Leads",
  subtitle = "Overall incoming leads captured across all active channels and campaigns",
  totalLeads = 463,
  targetLeads = 550,
  newLeads = 74,
  qualifiedLeads = 138,
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Target: {formatInteger(targetLeads)} leads
        </span>
      ...
      >
        <MetricTile
          label="New Leads"
          value={formatInteger(newLeads)}
          tone={toneStyles}
        />
        <MetricTile
          label="Qualified"
          value={formatInteger(qualifiedLeads)}
          tone={toneStyles}
        />
        <MetricTile
          label="Gap to Target"
      ...
      >
        ...
        >
          Lead health: {totalLeads >= targetLeads ? "Ahead of pace" : "Building momentum"}
        </span>
      </div>
    </section>
  );
